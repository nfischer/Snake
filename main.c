    ...
    while((ch = getch()) != 'x' && ch != 'q'){
        ...
        switch(game_state){
            case PLAY:
                if(ch != ERR){
                    switch(ch){
                    // if were moving make sure we dont move back to
                    // collide with the part right behind the head
                    case KEY_UP:
                        if(cur_dir != DOWN)
                            cur_dir = UP;
                        break;
...
void set_food_coord(Snake *snakey, Food* byte){
    ...
    do{
        ...
        newx = (rand() % (WORLD_WIDTH-2 ))+1;
        newy = (rand() % (WORLD_HEIGHT-2))+1;
        //if(newx == 0)
        //    newx++;
